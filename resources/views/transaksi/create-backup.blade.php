...
@section('scripts')
    ...
    <script>
        ...
        function printResit() {
            const resitContent = $('#print-div')[0];
            html2canvas(resitContent, {
                scale: 3, // Reasonable quality
                useCORS: true
            }).then(canvas => {
                ...
                // Calculate image size to fit page width with 4mm padding on each side
                var pageWidth = doc.internal.pageSize.getWidth();
                var pageHeight = doc.internal.pageSize.getHeight();
                var padding = 4;
                var imgWidth = pageWidth - 2 * padding;
                var imgProps = doc.getImageProperties(img);
                var imgHeight = (imgProps.height * imgWidth) / imgProps.width;
                // Ensure image does not exceed page height minus padding
                if (imgHeight > pageHeight - 2 * padding) {
                    imgHeight = pageHeight - 2 * padding;
        ...
        function saveResit() {
            ...
            swal.fire({
                ...
                if (result.isConfirmed) {
                    saveTransaction(pembeliId);
                }
            });
        }

        function saveTransaction(pembeliId) {
            $.ajax({
                type: "POST",
                url: "/transaksis",
                data: {
